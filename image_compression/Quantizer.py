...
from quantize_utils import AimetDataset
    ...
    def _apply_quantization(self, code):
        ...
        # compress with arithmetic coding
            ...
            bytes, frequencies = ae.compress_bytes(io.BytesIO(weights_np.tobytes()), out, bits=self.bitwidth,
                                                   write_freq=True)
            # we compress the step_sizes here as well with the general purpose brotli algorithm
            step_size_comp = brotli.compress(step_sizes.tobytes())
...
class ImageQuantizer(Quantizer):
    '''Quantizer class for images'''
    ...
    def __init__(self, model, dataset, dataloader, bitwidth, device, exp_path):
        ...
        self.image_resolution = dataset.sidelength
        self.input_shape = (2,)
        self.dataloader = DataLoader(dataset, shuffle=True, batch_size=1, pin_memory=True,
                                     num_workers=0)
        ...
        self.aimet_dataloader = DataLoader(AimetDataset(dataset), shuffle=False, batch_size=1, pin_memory=True,
                                           num_workers=0)
    ...
    def check_metrics(self):
        return utils.check_metrics(self.dataloader, self.sim.model, self.image_resolution)

    def retrain(self, epochs, lr):
        '''Retraining loop using quantization aware training'''
