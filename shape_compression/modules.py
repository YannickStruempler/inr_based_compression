...
class BatchConv(nn.Conv2d, MetaModule):
    '''A linear meta-layer that can deal with batched weight matrices and biases, as for instance output by a
    hypernetwork. (https://github.com/vsitzmann/siren)'''
...
########################
# Initialization methods (https://github.com/vsitzmann/siren)
...
def sine_init_conv(m):
    with torch.no_grad():
        if hasattr(m, 'weight'):
            num_input = m.weight.size(-3)
            # See supplement Sec. 1.5 for discussion of factor 30
            m.weight.uniform_(-np.sqrt(6 / num_input) / 30, np.sqrt(6 / num_input) / 30)


def first_layer_sine_init_conv(m):
    with torch.no_grad():
        if hasattr(m, 'weight'):
            num_input = m.weight.size(-3)
            # See paper sec. 3.2, final paragraph, and supplement Sec. 1.5 for discussion of factor 30
